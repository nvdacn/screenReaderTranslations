...
outFile=userGuide-stats-diff.txt
if [ "$nextRev" == "" ]; then
...
python ../scripts/stats.py userGuide-diffs/$nextRev/userGuide.t2t >next.stats
lang=$(basename `pwd`)

d_args=(-d --unchanged-line-format='' --old-line-format='en %L' --new-line-format="$lang %L")
sed_args=(
 # insert header line
 -e "1i# Structural comparison of ${lang}/userGuide.t2t against ${lang}/userGuide-diffs/$nextRev/userGuide.t2t"
